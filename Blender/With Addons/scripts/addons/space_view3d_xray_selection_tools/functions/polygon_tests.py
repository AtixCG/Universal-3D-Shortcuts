...
def segments_intersect_rectangle(segment_co, xmin, xmax, ymin, ymax, prefilter=False):
    ...
    with np.errstate(invalid="ignore"):
        segments_mask_not_isect = np.isnan(v1x) | np.isnan(v2x) | \
                        ((d1 < 0) & (d2 < 0) & (d3 < 0) & (d4 < 0)) | \
                        ((d1 > 0) & (d2 > 0) & (d3 > 0) & (d4 > 0))
    if prefilter:
...
def segments_inside_or_intersect_circle(segment_co, center, radius, prefilter=False):
    ...
    # closest_to_line
    # find closest point to circle center on line through x,y and find param
...
def segments_intersect_polygon(segment_co, poly, prefilter=False):
    ...
    poly_sides = len(poly)
    j = poly_sides - 1
    poly_segments_mask_isect_segments = []
    ...
    for i in range(poly_sides):
        ...
        div = (dx1 * dy2 - dy1 * dx2)
        # don't check the segments for collinearity, where div == 0
        with np.errstate(invalid="ignore", divide="ignore"):
        ...
        poly_segments_mask_isect_segments.append(poly_segments_mask_isect_segment)
        j = i
